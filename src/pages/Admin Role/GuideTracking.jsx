      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-gray-600 mt-1 text-sm">
              Track active guides in real-time
            </p>
          </div>
          <button
            onClick={() => {
            ...
            className="w-full sm:w-auto px-4 sm:px-5 py-2.5 sm:py-3 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl font-bold hover:from-violet-700 hover:to-purple-700 transition-all shadow-lg flex items-center justify-center gap-2 text-sm sm:text-base"
          >
            <RefreshCw size={17} />
            Refresh
          </button>
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-6">
          {[
            ...
            <div
              key={label}
              className="bg-white rounded-2xl p-3 sm:p-6 shadow-md border-2 border-violet-100"
            >
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <div
                  className={`w-9 h-9 sm:w-12 sm:h-12 ${bg} rounded-xl flex items-center justify-center`}
                >
                  {icon}
                </div>
                {pulse && (
              ...
              <p className="text-[10px] sm:text-sm font-semibold text-gray-600 mb-0.5 sm:mb-1 leading-tight">
                {label}
              </p>
        ...
        <div className="bg-white rounded-2xl shadow-md border-2 border-violet-100 overflow-hidden">
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-violet-100 flex items-center justify-between">
            <div>
            ...
            {liveCount > 0 && (
              <div className="flex items-center gap-1.5 sm:gap-2 text-green-600 text-xs sm:text-sm font-semibold">
                    ...
                    <Popup>
                      <div className="text-sm min-w-[140px]">
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                          Updated: {loc.lastSeen?.toLocaleTimeString()}
                        </p>
                      </div>
                    </Popup>
                  </Marker>
        ...
        {liveCount === 0 ? (
          <div className="bg-white rounded-2xl shadow-md p-8 sm:p-12 text-center border-2 border-violet-100">
            ...
            />
            <h3 className="text-base sm:text-xl font-bold text-gray-400 mb-1 sm:mb-2">
              No Active Guides
            </h3>
            <p className="text-gray-500 text-sm">
              No guides are currently sharing their location
            </p>
          </div>
        ) : (
          <div className="bg-white rounded-2xl shadow-md border-2 border-violet-100 overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {Object.entries(liveLocations).map(([guideId, loc]) => (
                <div
                  ...
                  className="p-3 sm:p-6 hover:bg-violet-50 transition-colors cursor-pointer"
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                      <div className="relative flex-shrink-0">
                        <div className="w-10 h-10 sm:w-14 sm:h-14 bg-gradient-to-br from-violet-600 to-purple-600 rounded-full flex items-center justify-center text-white font-black text-sm sm:text-xl shadow-lg">
                          {getInitials(loc.username)}
                        </div>
                        <span className="absolute -top-0.5 -right-0.5 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-green-500 rounded-full border-2 border-white animate-pulse" />
                      </div>
                      <div className="min-w-0">
                        <h3 className="font-black text-gray-900 text-sm sm:text-lg truncate">
                          {loc.username || `Guide #${guideId.slice(-6)}`}
                        </h3>
                        {loc.email && (
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border-2 border-violet-200 max-h-[90vh] flex flex-col">
              ...
              <div className="p-4 sm:p-6 space-y-3 sm:space-y-4 overflow-y-auto">
                <div className="flex items-center gap-3 sm:gap-4 pb-3 sm:pb-4 border-b border-gray-100">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-violet-600 to-purple-600 rounded-2xl flex items-center justify-center text-white font-black text-xl sm:text-2xl shadow-lg flex-shrink-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                      ...
                      {selectedGuide.location && (
                        <span className="px-2.5 py-1 bg-violet-100 text-violet-700 rounded-full text-xs font-bold truncate max-w-[140px] sm:max-w-[160px]">
                          📍 {selectedGuide.location}
                        </span>
                      )}
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                  ...
                  <div className="col-span-1 sm:col-span-2 bg-gray-50 rounded-xl p-3">
                    ...
                    <p className="text-xs sm:text-sm font-semibold text-gray-900 font-mono">
                  ...
                  <div className="col-span-1 sm:col-span-2 bg-gray-50 rounded-xl p-3">
                    ...
                    <p className="text-xs sm:text-sm font-semibold text-gray-900">
                      {liveLocations[
                        selectedGuide._id
                      ]?.lastSeen?.toLocaleTimeString() ?? "N/A"}
                    </p>
                  </div>
                </div>
              </div>

              <div className="p-3 sm:p-6 bg-gray-50 border-t flex flex-col sm:flex-row gap-2 sm:gap-3 flex-shrink-0">
                <button
                  onClick={() => handleClearGuide(selectedGuide._id)}
                  className="flex-1 py-2.5 sm:py-3 rounded-xl bg-red-50 text-red-600 font-bold border border-red-200 hover:bg-red-100 transition-all text-sm"
                >
                  Remove from List
                </button>
                <button
                  onClick={() => setSelectedGuide(null)}
                  className="flex-1 py-2.5 sm:py-3 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 text-white font-bold hover:from-violet-700 hover:to-purple-700 transition-all text-sm"
                >
                  Close
                </button>
              </div>
